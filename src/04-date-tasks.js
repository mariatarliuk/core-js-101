...
function timeSpanToString(startDate, endDate) {
  ...
  if (sec < 10) sec = `0${sec}`;
  let milli = endDate.getMilliseconds() - startDate.getMilliseconds();
...
function angleBetweenClockHands(date) {
  const hour = 0.5 * (60 * (new Date(date).getHours() - 3) + new Date(date).getMinutes());
  const min = 6 * new Date(date).getMinutes();
